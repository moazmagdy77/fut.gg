// shared.fetch.data.js (Turbo Edition)
// High-Performance Parallel Fetcher
// Optimizations: Concurrency 25, Async I/O, Stealth, Persistent Browser
...
// --- Process Arguments ---
// Example usage: node shared.fetch.data.js --inputFile ../data/club_ids.json --mode club
const args = process.argv.slice(2);
let INPUT_FILE = '../data/club_ids.json';
let MODE = 'club';

for (let i = 0; i < args.length; i++) {
    if (args[i] === '--file' || args[i] === '--inputFile') {
        INPUT_FILE = args[i+1];
        i++;
    } else if (args[i] === '--mode') {
        MODE = args[i+1];
...
async function fetchPuppeteer(page, url, type) {
    for (let i = 0; i <= RETRIES; i++) {
        try {
            ...
            if (i === RETRIES) {
                // Silent fail on max retries to keep logs clean
                return null;
            }
        }
    }
...
(async () => {
    ...
    console.log(`🚀 Starting TURBO Data Fetcher ${logPrefix}...`);
    ...
    try {
        ...
        console.error(`❌ Could not load ${INPUT_FILE}: ${e.message}`);
        return;
    }
    ...
    try {
        ...
        console.error(`❌ Could not load maps.json: ${e.message}`);
        return;
    }
