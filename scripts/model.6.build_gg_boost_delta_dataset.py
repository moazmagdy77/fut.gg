...
OUTPUT_FILE = BASE_DATA_DIR / 'training_dataset_gg_boost_delta.csv'
...
def main():
    print("🚀 Starting script to build the ggMeta BOOST DELTA training dataset...")
    ...
    for i, ea_id_str in enumerate(player_ids):
        ...
        for role_id_str, scores in scores_by_role.items():
            ...
            # The features for this model are the player's BASE stats plus the chem style
            row = base_features.copy()
            row.update(base_attributes)
            row["role"] = role_name

            # Create a row for every chem style, where the target is the difference from basic
            for score_entry in scores:
                ...
                final_row["target_ggMetaBoostDelta"] = score_entry.get("score", basic_score) - basic_score
                all_rows.append(final_row)

    print("✅ Finished processing. Creating final DataFrame...")
    df = pd.DataFrame(all_rows).dropna(subset=['target_ggMetaBoostDelta']).fillna(0)
    df = pd.get_dummies(df, columns=["role", "bodytype", "foot", "chem_style"], dtype=int)
    
    df.to_csv(OUTPUT_FILE, index=False)
    print(f"💾 Saved ggMeta boost delta training data with {len(df)} rows to {OUTPUT_FILE.name}")
