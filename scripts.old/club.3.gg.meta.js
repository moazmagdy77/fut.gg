...
const fetchMetarating = async (browser, id, retries = 2) => {
  const apiUrl = `https://www.fut.gg/api/fut/metarank/player/${id}/`;
  console.log(`📦 Fetching metarating for player ID ${id}`);
...
(async () => {
  ...
  for (let i = 0; i < playerIds.length; i += MAX_CONCURRENT) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (id) => {
        const json = await fetchMetarating(browser, id);
        console.log(`📥 Finished ID ${id}`);
        if (json) metaratingData.push(json);
      })
    );
  ...
  console.log(`💾 Writing metarating output to file...`);
  try {
    await Promise.race([
      new Promise((resolve, reject) => {
        fs.writeFile(path.join(dataDir, 'club_metarank.json'), JSON.stringify(metaratingData, null, 2), (err) => {
    ...
    console.log(`💾 Successfully wrote metarating file.`);
  } catch (err) {
    console.error(`❌ Failed to write metarating file: ${err.message}`);
  }
  ...
  console.log(`✅ Saved metarating data for ${metaratingData.length} players to club_metarank.json`);
  process.exit(0);
})();